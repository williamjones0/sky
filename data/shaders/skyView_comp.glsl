 ...
 * @param worldDirection - the direction in which the shift will be done
 * @param atmosphereBoundaries - x is bottom radius, y is top radius
 */
...
layout (binding = 5, rgba16f) uniform image2D skyViewLUT;
/* ================================== NOT USED ==================================== */
...
void main()
{
    ...
    vec3 sunDirection = atmosphereParameters.sun_direction.xyz;
...
//    imageStore(skyViewLUT, ivec2(gl_GlobalInvocationID.xy), vec4(Luminance, 1.0));
    imageStore(skyViewLUT, ivec2(gl_GlobalInvocationID.x, atmosphereParameters.SkyViewTexDimensions.y - gl_GlobalInvocationID.y), vec4(Luminance, 1.0));
}
